...
The `messaging/` folder contains a standalone web app (`messaging/index.html`) with two views:

- **Chats** — a WhatsApp-style messaging interface for ColourDiam (contact list, chat bubbles, input bar, read ticks) with sample conversations saved locally. It demonstrates the customer-facing experience powered by the n8n workflows.
- **Connector** — a setup wizard that walks you through connecting every platform: enter your n8n URL, API key and verify token, enter each platform's tokens (WhatsApp, Instagram, Facebook, TikTok, LINE, WeChat), auto-built webhook URLs (copy button), per-platform setup steps, a generated ready-to-use `.env` file (copy button) and the n8n import command.

Config is saved locally in your browser — tokens never leave your device. Open it by serving the `messaging/` folder or via GitHub Pages.
