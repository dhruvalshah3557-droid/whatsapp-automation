...
- [x] 260807: Auto-upload ALL app data to Cloudflare (dual-write memory backup) — status: finished
  - Done: the app's server-backed memory already snapshotted every localStorage bucket (chat, ui, config, auth, reminders, replies, AI, media) to the *current* server — which for the preview is the local server, so nothing reached Cloudflare; `backupMemory()` now writes to BOTH the configured server AND the Cloudflare worker (`https://messaging-webhooks.messaging-webhooks-worker.workers.dev`, the `CLOUD_BACKUP_URL` constant) on every save (debounced 1.5s, skips the duplicate when the worker is already the server), and `restoreMemory()` fetches both and restores the newest copy (by `at` timestamp), so data survives local shutdown / preview URL changes; verified live: `POST /api/memory` → 200 and `GET /api/memory` returns the data from the worker's `app-memory` KV; SW bumped to `colourdiam-msg-v35`; 4 new messaging tests (10 → 14); MONITOR OK (127/127 tests).
...
- [x] 260807: Port auth/tasks/admin to the Cloudflare Worker (KV-backed) so the production app survives local shutdown — status: finished
  ...
  - Fixed deploy failure: first deploy (commit `ee668f3`) failed with KV namespace invalid (code 10042) because `AUTH_KV_PLACEHOLDER` contains `KV_PLACEHOLDER` as a substring, so the first `sed` corrupted the AUTH id to `AUTH_<events-id>`; renamed placeholders to `EVENTS_KV_ID` / `AUTH_KV_ID` (commit `e387b12`) so each replaces independently — deploy succeeded with both `EVENTS` and `AUTH` KV bindings.
  - Verified live on `messaging-webhooks.messaging-webhooks-worker.workers.dev`: `/api/auth/login` with `admin@colourdiam.com`/`Admin2026!` bootstraps the admin (200 + token, `role:"admin"`), `/api/auth/me` returns the user for the token, forgot/reset/activity/admin all respond correctly; sent a test message through `/webhook/facebook-hook` (200) and confirmed it landed in `/api/events` (event id 2, `from: test_customer_1`), `/api/health` events 1 → 2.
